 ...
 *
 * One form serves both sides of the trade — whichever role is logged in
 * (seller or buyer) becomes that party on the deal, and the other party's
 * slot (seller_id/buyer_id) is left null until they join via the invite link
 * sent below. Dealers/brokers/admins keep the existing referral-creation path
...
async function createDeal(req, res) {
  ...
  const creatorRole = req.appUser.role;
  const isPartyCreator = creatorRole === 'seller' || creatorRole === 'buyer';
  ...
  if (product === 'loanclear' && creatorRole === 'seller' && (typeof loanAmount !== 'number' || loanAmount < 0)) {
  ...
  if (isPartyCreator) {
    const otherRole = creatorRole === 'seller' ? 'buyer' : 'seller';
...
async function joinDeal(req, res) {
  ...
  if (req.appUser.role !== role) {
    return res.status(403).json({ error: `This invite is for a ${role} account — you're logged in as a ${req.appUser.role}` });
  }
