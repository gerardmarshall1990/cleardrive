...
export default function BuyerDealDetail({ route }) {
...
function KycCard({ deal }) {
  return (
    <DarkCard style={{ marginTop: 24 }}>
      ...
      {!deal.buyer_kyc_complete && <Text style={[styles.cardBody, { marginTop: 12 }]}>Check WhatsApp for a link to complete your identity verification.</Text>}
      {deal.buyer_kyc_complete && !deal.seller_kyc_complete && <Text style={[styles.cardBody, { marginTop: 12 }]}>You're verified — waiting on the seller.</Text>}
    </DarkCard>
  );
}
...
function SigningCard({ deal }) {
  ...
  return (
    <DarkCard style={{ marginTop: 24 }}>
      ...
      <Text style={[styles.cardBody, { marginBottom: 12 }]}>Check your email for signing links. We'll notify you once everyone has signed.</Text>
      <View style={{ gap: 10 }}>
        {docs.map((doc) => (
...
function EscrowCard({ deal }) {
  return (
    <DarkCard style={{ marginTop: 24 }}>
      ...
      <Text style={[styles.cardBody, { marginBottom: 12 }]}>Transfer the agreed sale price to the secure escrow account below.</Text>
